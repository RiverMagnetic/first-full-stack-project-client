...
var showItemsTemplate = __webpack_require__(15);
...
var onCreateItemSuccess = function onCreateItemSuccess() {
...
var onUpdateItemSuccess = function onUpdateItemSuccess() {
...
var itemEvents = __webpack_require__(33);
...
var itemUi = __webpack_require__(6);
...
var onSignOut = function onSignOut(event) {
    ...
    authApi.signOut().then(authUi.signOutSuccess).then(itemUi.clearItems).catch(authUi.signOutFailure);
};
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return " "
    ...
    + "\">\n    <small>"
    + alias2(alias3(((stack1 = blockParams[0][0]) != null ? stack1.item_name : stack1), depth0))
    + "</small>\n</h1>\n\n<ul data-id=\""
    + alias2(alias3(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n    <li><h2>"
...
var onGetItems = function onGetItems(event) {
    event.preventDefault();
    Promise.resolve(itemUi.clearItems).then(itemApi.getItems).then(itemUi.getItemsSuccess).catch(itemUi.onError);
};

var onClearItems = function onClearItems(event) {
    event.preventDefault();
    itemUi.clearItems();
};
...
var onDeleteItem = function onDeleteItem(event) {
    event.preventDefault();
    var itemId = $(event.target).closest('ul').attr('data-id');
    itemApi.deleteItem(itemId)
    // may need refactoring
    .then(itemUi.onDeleteItemSuccess).then(function () {
        return onClearItems(event);
    }).then(function () {
...
var onUpdateItem = function onUpdateItem(event) {
    ...
    if (item.id.length !== 0) {
        ...
        // console.log(data)
        itemApi.updateItem(data).then(itemUi.onUpdateItemSuccess).then(function () {
            return onClearItems(event);
        }).then(function () {
            return onGetItems(event);
        }).catch(itemUi.onError);
...
var onCreateItem = function onCreateItem(event) {
    ...
    // console.log(data)
    // API call
    itemApi.createItem(data).then(function () {
        return onClearItems(event);
    }).then(function () {
        return onGetItems(event);
    }).then(itemUi.onCreateItemSuccess).fail(itemUi.onError);
